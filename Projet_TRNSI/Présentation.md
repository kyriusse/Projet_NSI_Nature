...
Ce projet a été réalisé par Armand Jardiller-Dexpert / Eguzki Bailli-ayrault / Romain Lacornette / Tom Anglade / en classe de NSI du Lycée Champollion avec l'aide de Mr Jalras enseignant. L’idée était de relier la nature et l'informatique en utilisant des graphes pour représenter des intéractions entre différents éléments pour par la suite potentiellement prédire les conséquences de ceux-ci sur la nature.

Enfaite l'idée vient de quelque choses de beaucoup plus complexe. Depuis le début de l'année de NSI, nous associons le programme à des choses du quotidien un peu comme une blague. Après la réalisation d'un projet sur le SQL, Privealy, permettant d'obtenir le prix moyen de n'importe quel objet et de simuler son évolution sur 20 ans. Nous avons eu l'envie de pousser le projet plus loin en créant une vraie simulation économique. Nous nous sommes rendu compte que ce qui impacter le plus l'économie était l'environnement. Aussi, cela est réciproque, alors apprenant le thème des trophées NSI, nous nous sommes lancés dans l'amélioration de cette simulation. Plus on avancait, plus on voulait être réaliste, plus le projet devenait un enfer. On a donc décidé, de tout généraliser. Avant ça nous avons beaucoup réfléchit sur nos observations des liens entre notre monde / quotidien et le programme de NSI, du lien entre l'économie et la nature et voici ce que nous avons conclu :
...
\-Avoir des résultats visuelles / Pouvoir positionner ces objets dans un espace simuler (on à appelé ça, les objets réels).


**Présentation de l'équipe**

Nous sommes 4 élèves de terminale :
...
- Le modèle définit (objets, liaisons, événements...), nous avons hierarchisez les concepts du plus essentiel au moins essentiel afin de procéder de manière évolutive => C'est à dire de définir des sortes d'étapes où le projet pourrait s'arrêter. On a appellé ces étapes des "checkpoint". Cette méthode nous a beaucoup aidée.
...
- Ensuite nous avons rétablit les tâches à l'intérieur de l'équipe, en prenant en compte de l'envie, des compétences et disponibilités de chacun.
...
- Création du site avec Flask => Toute les pages sont crées, tout les boutons existent, mais le code n'agit pas encore.

- création de la simulations (+ Donées, Affichage du Graphe, Ajouts rapides)

- Finition de toute les pages et affichage (Journal de bord simulation, composition, Courbe)
...
- Création de tests et exemples plus complexes, entièrement codé en Frank SQL (FSQL).
...
Nous avons donc créer le moteur et son langage pour pouvoir créer notre simulation rêvée. 
...
Nous avons tester sur d'autres machines. 

Nous faisions donc des tests réguliers et des observations rigoureuses sur des simulations. On a lancé des projets de simulations complexe pour mettre le projet à l'épreuve. 
...
- propagation dans le graphe -> ajout d’une profondeur et d'une conservation (à quel point la valeur est conserver dans le graphe) et pas tout en une liaison. 
autrement dit on a diviser la propagation en 2 (nombre de sommets ; conservations)
- Positionnement de points sur la carte -> JS 
- Evénement périodique -> Création des paternes qui associent une action à une certaine fréquence.
...
- Pouvoir intepréter des matrice d'adj, afin que les utilisateurs puissent partager leur liaisons directements avec la matrice, ou la liste d'adj.
- Pouvoir obtenir des dictionnaires de liste d'adj.
- Final : Faire des tables dans des tables (une vrai programation objets améliorant cp{} et la page composition). Les tables dans des tables serait enfaite d'autres tables et le site les afficheraient comme des tables de tables avec un système de pointeur ou de liaison "=>", soit des tables "composants" d'autre tables. ((On se rapproche de plus en plus de la fractale, forme de la nature ;) )
...
Eguzki : 
Au départ j'ai eu un peu peur quand on a commencé à parler théorie. Heureusement j'ai su poser les limites avec eux. Cependant, je n'ai jamais eu une aussi bonne entente dans un groupe. J'ai été motivé tout au long du projet, avoir toutes ces responsabilités au niveau de la faisabilité n'était pas une mince à faire mais j'étais content que l'on m'ai choisit cela m'a fait apprendre beaucoup de choses, surtout au niveau de la gestion de groupe, de demandes, et d'erreurs potentielle. Peut importe les résultats, ce projet m'a beaucoup apprit, et cela ma apporté beaucoup d'expérience.

Défaut => Mauvaise décisions en situation de crise, conceptualiser avant de coder.

Tom :
Quand Armand m'a dit qu'on allait faire un site, je me suis directement mis à apprendre du HTML. C'est un langage que j'apprends depuis ma découverte de celui-ci en Première par M. Carolo. La gestion d'autant de fichier est un peu stressante, mais le cadre du groupe et surtout les fonction et rôles distribuer à chacun ainsi que le système complexe d'organisation que l'on a mis en place est vraiment rassurant. J'ai appris de ce système, j'ai appris le HTML comme jamais au par avant. J'ai eu beaucoup d'erreur mais avec le recul, leur correction à permit à mon code de devenir beaucoup plus rigoureux mais surtout libre. En effet je peux maintenant faire un projet sans consulter la documentation toute les 5 min ou reprendre des exemples types.

Défaut => Organisation / les code type pile, file ou encore la POO. Aussi l'écoute, j'ai appris à écouter tout le monde.

Armand :
Conceptualiser un projet était déjà fascinant mais le rendre réel ce n’était pas une mince à faire. Avoir un groupe à l'écoute aide beaucoup. Je n'ai jamais autant appris et fait de lien logique avec le programme de NSI que durant ce projet (sincèrement). Imaginer un langage de programmation, et le faire ça m'a ouvert les portes de choses encore plus complexes. Gérer un groupe ou une équipe ce n’était pas évident mais celui-ci était vraiment sympas, la gestion de crise, et l'organisation rigoureuse m'ont beaucoup appris. Trouver des solutions logique aux problèmatiques était un exellent entraînement. Savoir reconaitre ses erreurs, est très important pour faire avancer le projet et pour apprendre, c'est l'une des choses que j'ai apprises.

défaut => Organisation et communication d'idées, programation type et non brouillon.

Romain :
C'était tellement compliqué, la carte tout ça. Mais travailler avec Armand et les autres c'était plutôt facile. L'organisation était très spéciale (hyper bien définit) et les membres du groupes m'ont poussés vers le haut. J'ai appris sur le JS (ce n'était pas le but mais je souhaite le souligner). J'ai aussi appris à conceptualiser des choses hyper abstraites comme un repère dans un espace sans représentation visuelle. Travailler sur un code aussi gros, se rappeler de ce qu'on à fait le mois dernier ect... C'était vraiment dur mais avec une bonne communication (j'ai appris à utiliser GitHub !) sa marche super bien. Pour être honnête au début je pensais que j'allais utiliser l'IA, mais je ne suis pas peu fièr de vous dire que 0% de mon travail l’utilise. Aussi, quand quelqu'un finissait un code il faisait un dossier court de toute les fonctions qu'il avait utilisé et pourquoi, même si je n'étais pas concerné par ce code je lisais à chaque fois, et j'ai appris énormément de chose. Sincèrement je pense avoir fait un évolution fulgurante en programmation, surtout ces dernier jours. Et non je n'exagère pas.
