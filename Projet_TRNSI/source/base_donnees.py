...
def type_sql_pour_nsi(type_colonne):
    if type_colonne == 'int':  #si le type est int
        return 'INTEGER'  #retourne le type SQL correspondant
    if type_colonne == 'float':  #si le type est float
...
def creer_tables():
    ...
    #table qui garde la liste des colonnes de chaque table Créer
...
def liste_tables_nsi():
    #retourne toutes les tables existantes, triées par principale puis par nom
...
def type_nsi_colonne(nom_table, nom_colonne):
    #retourne le type d'une colonne d'une table donnée
...
def liste_colonnes_table(nom_table):
    #retourne la liste de toutes les colonnes d'une table triées par position
...
def ajouter_colonne_table(nom_table, nom_colonne, type_colonne='texte', valeur_defaut=''):
    #ajoute une nouvelle colonne à une table existante
    ...
    type_sql = type_sql_pour_nsi(type_colonne)  #convertit le type en type SQL
...
def ajouter_objet_table(nom_table, donnees):
    ...
    for nom_colonne in colonnes_physiques(nom_table):
        ...
        if nom_colonne == 'nom':
            ...
            continue
        type_colonne = type_nsi_colonne(nom_table, nom_colonne)  #récupère le type
