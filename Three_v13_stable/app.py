...
from simulation import formater_temps, simuler, svg_graphique
...
def page_donnees():
        ...
        try:
            if action == 'creer_table':
                ...
                for ligne in base_donnees.liste_colonnes_table(nom_table):
                    donnees[ligne['nom_colonne']] = request.form.get(ligne['nom_colonne'], ligne['valeur_defaut'])
                ...
                if bilan_liaisons > 0:
                    flash('Ajout rapide termine : objets et ' + str(bilan_liaisons) + ' liaison(s).', 'succes')
                else:
                ...
                base_donnees.ajouter_inversion(request.form.get('valeur_0', ''), request.form.get('valeur_1', ''))
                ...
                base_donnees.supprimer_inversion(entier_ou_defaut(request.form.get('id_inversion', '0'), 0))
    ...
    return render_template('donnees.html', tables=tables, table_active=nom_table, objets=objets, colonnes=colonnes, colonnes_physiques=colonnes_physiques, unites=unites, inversions=inversions)
...
def page_liaisons():
    if request.method == 'POST':
        ...
        try:
            if action == 'ajout_rapide':
                ...
                base_donnees.supprimer_liaison(entier_ou_defaut(request.form.get('id_liaison', '0'), 0))
    ...
    return render_template('liaisons.html', objets_catalogue=objets_catalogue, liaisons=liaisons, svg=svg_graphe(objets_catalogue, graphe, poids))
...
def page_composants():
    if request.method == 'POST':
        ...
        try:
            if action == 'ajouter_composant':
                base_donnees.ajouter_composition(request.form.get('parent_nom', ''), request.form.get('enfant_nom', ''), entier_ou_defaut(request.form.get('quantite', '1'), 1))
                try:
                    base_donnees.ajouter_liaison(request.form.get('enfant_nom', ''), request.form.get('parent_nom', ''), '=>', 1, 'n')
                except Exception:
                    pass
                ...
                for morceau in morceaux:
                    ...
                    if '(' in morceau and morceau.endswith(')'):
                        ...
                        quantite = entier_ou_defaut(morceau[morceau.find('(')+1:-1], 1)
                ...
                base_donnees.supprimer_composition(entier_ou_defaut(request.form.get('id_composition', '0'), 0))
                ...
                if base_donnees.objet_global_par_nom(nom_resultat) is None:
                    base_donnees.ajouter_objet_table(table, {'nom': nom_resultat, 'valeur': 0, 'etat': ''})
    ...
    return render_template('composants.html', objets_catalogue=objets_catalogue, compositions=compositions, arbre=arbre, racine=racine, max_branches=max_branches)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if action == 'changer_etat_evenement':
                base_donnees.changer_etat_evenement(entier_ou_defaut(request.form.get('id', '0'), 0), entier_ou_defaut(request.form.get('actif', '1'), 1))
                return redirect(url_for('page_simulation'))
            if action == 'supprimer_evenement':
                base_donnees.supprimer_evenement(entier_ou_defaut(request.form.get('id', '0'), 0))
                flash('Evenement supprime.', 'succes')
                return redirect(url_for('page_simulation'))
            if action == 'changer_etat_paterne':
                base_donnees.changer_etat_paterne(entier_ou_defaut(request.form.get('id', '0'), 0), entier_ou_defaut(request.form.get('actif', '1'), 1))
                return redirect(url_for('page_simulation'))
            if action == 'supprimer_paterne':
                base_donnees.supprimer_paterne(entier_ou_defaut(request.form.get('id', '0'), 0))
            ...
            if action == 'simuler':
                ...
                historique, courbes = simuler(objets, liaisons, evenements, paternes, inversions, nb_tours, selection_objets, valeur_tour, unite_tour, dictionnaire_unites(unites))
                resultat = {'historique': historique, 'graphique': svg_graphique(courbes)}
        except Exception as exc:
            flash(str(exc), 'erreur')
...
def page_code():
    ...
    return render_template('code.html', code=code, bilan=bilan, code_colore=code_colore)
...
def page_map():
    ...
    if request.method == 'POST':
        try:
            ...
            if action == 'enregistrer_position':
                base_donnees.enregistrer_position(entier_ou_defaut(request.form.get('objet_id', '0'), 0), reel_ou_defaut(request.form.get('latitude', '0'), 0), reel_ou_defaut(request.form.get('longitude', '0'), 0), request.form.get('lieu', ''))
                flash('Position enregistree.', 'succes')
            elif action == 'supprimer_position':
                base_donnees.supprimer_position(entier_ou_defaut(request.form.get('id_position', '0'), 0))
                flash('Position supprimee.', 'succes')
            elif action == 'supprimer_chemin':
                base_donnees.supprimer_chemin(entier_ou_defaut(request.form.get('id_chemin', '0'), 0))
    ...
    for ligne in positions:
        ...
        points.append({'nom': ligne['nom'], 'x': x, 'y': y, 'lieu': ligne['lieu']})
    chemin_selectionne = None
    points_chemin = []
    if chemin_id:
        points_chemin = vers_dicts(base_donnees.points_chemin(entier_ou_defaut(chemin_id, 0)))
        if points_chemin:
            chemin_selectionne = entier_ou_defaut(chemin_id, 0)
    return render_template('map.html', objets_catalogue=objets_catalogue, positions=positions, points=points, chemins=chemins, chemin_selectionne=chemin_selectionne, points_chemin=points_chemin)
